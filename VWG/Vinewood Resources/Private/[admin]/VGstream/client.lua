...
addEventHandler( "onClientGUIClick", DENStreamButton1,
	function ()
		if ( isStreaming ) then
			exports.VGdx:showMessageDX( "You are already streaming, stop the first stream first!", 225, 0, 0 )
		elseif ( getStreamURL () == "" ) or ( getStreamURL () == " " ) then
			exports.VGdx:showMessageDX( "Can't start stream, please enter a valid URL", 225, 0, 0 )
		else
			-- local testSound = playSound ( getStreamURL(), 0, 0, 0 )
...
addEventHandler( "onClientGUIClick", DENStreamButton2,
	function ()
		if ( isStreaming ) then
			...
			exports.VGdx:showMessageDX( "You aren't even streaming!", 225, 0, 0 )
		end
	end, false
)
	...
	function ( soundTable, aPlayer )
		...
		if ( aPlayer == localPlayer ) then
			if not ( theSound ) then
				exports.VGdx:showMessageDX( "Stream URL seems not to work properly!", 225, 0, 0 )
			end
			isStreaming = true 
		end
	end
