...
function setWindow( elm )
	guiSetVisible( elm, true )
	guiWindowSetMovable( elm, true )
	guiWindowSetSizable( elm, false )
	guiSetVisible( elm, false )

	local screenW, screenH = guiGetScreenSize()
	local windowW, windowH = guiGetSize( elm, false )
	local x, y = ( screenW - windowW ) / 2,( screenH - windowH ) / 2
	guiSetPosition( elm, x, y, false )
end

centerWindow = setWindow

-- Set all the fonts
function setOrangeFont( elm )
	guiSetFont( elm, "default-bold-small" )
	guiLabelSetColor( elm, 245, 175, 9 )
	guiLabelSetHorizontalAlign( elm, "center", false )
	guiLabelSetVerticalAlign( elm, "center" )
end
	...
	function ()
		if ( guiGetEnabled( source ) ) and ( getAdminPanelSelectedPlayer () ) then
			...
			elseif ( source == DENAdminGUI.button[20] ) then
				triggerServerEvent( "onServerRequestPlayerLogins", localPlayer, getAdminPanelSelectedPlayer () )
			elseif ( source == DENAdminGUI.button[19] ) then
				triggerServerEvent( "onServerRequestPlayerPunishments", localPlayer, getAdminPanelSelectedPlayer () )
			...
			elseif ( source == DENAdminGUI.button[2] ) then
				triggerServerEvent( "onServerShowJailWindow", localPlayer, getAdminPanelSelectedPlayer () )
