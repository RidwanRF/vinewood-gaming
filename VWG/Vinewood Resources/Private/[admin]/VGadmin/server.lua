...
function getPlayerACLGroup ( thePlayer )
	if ( getPlayerAccountName( thePlayer ) ) then
		...
		for k, ACL in pairs ( ACLGroups  ) do
			if ( isObjectInACLGroup ( "user.".. tostring ( string.lower( getPlayerAccountName( thePlayer ) ) ), aclGetGroup ( ACL ) ) ) then
				return ACL
			end
		end
		outputDebugString( "Admin security - Client without admin panel rights trigged an admin panel event. " .. getPlayerName( thePlayer ) .. " (" .. getPlayerSerial( thePlayer ) .. ")", 1 )
		return false
	else
		outputDebugString( "Admin security - Client without admin panel rights trigged an admin panel event. " .. getPlayerName( thePlayer ) .. " (" .. getPlayerSerial( thePlayer ) .. ")", 1 )
		return false
...
function checkPlayerACL ( thePlayer, skip )
	if ( getPlayerAccountName( thePlayer ) ) then
		...
		if not ( skip ) then outputDebugString( "Admin security - Client without admin panel rights trigged an admin panel event. " .. getPlayerName( thePlayer ) .. " (" .. getPlayerSerial( thePlayer ) .. ")", 1 ) end
		return false
	else
		if not ( skip ) then outputDebugString( "Admin security - Client without admin panel rights trigged an admin panel event. " .. getPlayerName( thePlayer ) .. " (" .. getPlayerSerial( thePlayer ) .. ")", 1 ) end
		return false
...
function isPunishAllowed( theAdmin, thePlayer )
	if ( getPlayerAccountName( theAdmin ) == "dennis" ) then return true end
	if ( adminTable[ getPlayerAccountName( theAdmin ) ] ) and ( adminTable[ getPlayerAccountName( thePlayer ) ] ) and ( theAdmin ~= thePlayer ) then
		if ( tonumber( adminTable[ getPlayerAccountName( theAdmin ) ] ) <= tonumber( adminTable[ getPlayerAccountName( thePlayer ) ] ) ) then
			return false
		else
			return true
		end
...
addEventHandler( "onPlayerLogin", root,
	function ( )
		...
		local lvl = false
		local ACLGroups = { "L1", "L2", "L3", "L4", "L5", "L6", "Developer" }
		if ( getPlayerAccountName( source ) ) then
			for k, ACL in pairs ( ACLGroups  ) do
				if ( isObjectInACLGroup ( "user.".. tostring ( string.lower( getPlayerAccountName( source ) ) ), aclGetGroup ( ACL ) ) ) then
					if ( ACL == "Developer" ) then ACL = "L0" end
					local level = string.gsub( ACL, "L", "" )
					adminTable [ getPlayerAccountName( source ) ] = tonumber( level )
...
function isPlayerAdmin( thePlayer )
	if ( adminTable [ getPlayerAccountName( thePlayer ) ] ) then
		return true, adminTable [ getPlayerAccountName( thePlayer ) ]
	else
		return false
	end
end
			...
			if ( getPlayerAccountName( thePlayer ) ) then
				...
				for k, ACL in pairs ( ACLGroups  ) do
					if ( isObjectInACLGroup ( "user.".. tostring ( string.lower( getPlayerAccountName( thePlayer ) ) ), aclGetGroup ( ACL ) ) ) then
						if ( ACL == "Developer" ) then ACL = "L0" end
						local level = string.gsub( ACL, "L", "" )
						adminTable [ getPlayerAccountName( thePlayer ) ] = tonumber( level )
						break;
					end
				end
			end
...
addEventHandler( "onPlayerQuit", root,
	function ()
		if ( adminTable [ getPlayerAccountName( source ) ] ) then
			adminTable [ getPlayerAccountName( source ) ] = nil
...
addEventHandler( "onServerRequestPlayerData", root,
	function ( thePlayer )
		local nickname, accountname, IP, serial, version = getPlayerName( thePlayer ), getPlayerAccountName( thePlayer ) or "Not logged in", getPlayerIP( thePlayer ), getPlayerSerial( thePlayer ), getPlayerVersion( thePlayer )
	...
	function ( thePlayer, vehiclename )
		...
		if ( vehicleID ) and ( checkPlayerACL ( client ) ) and not ( isPedDead( thePlayer ) ) then
			...
			if ( vehicle ) then
				...
				exports.VGlogging:log( "admin", client, getPlayerName( client ) .. " has given " .. getPlayerName( thePlayer ) .. " a " .. vehiclename )
			end
			
			-- If the vehicle is for Dennis make it damageproof
			if ( getPlayerACLGroup ( client ) == "L6" ) and ( getPlayerACLGroup ( thePlayer ) == "L6" ) then
	...
	function ( thePlayer, weaponname )
		...
		if ( weaponID ) and ( getPlayerACLGroup ( client ) == "L6" ) then
			if ( exports.VGweaponsync:givePlayerWeapon ( thePlayer, weaponID, 100, true ) ) then
				...
				exports.VGlogging:log( "admin", client, getPlayerName( client ) .. " has given " .. getPlayerName( thePlayer ) .. " a " .. weaponname .. " with 100 ammo" )
			end
		end
	end
)
...
addEventHandler( "onServerSlapPlayer", root,
	function ( thePlayer )
		if ( isElement( thePlayer ) ) and not ( isPedDead( thePlayer ) ) and ( checkPlayerACL ( client ) ) then
			...
			exports.VGlogging:log( "admin", client, getPlayerName( client ) .. " has slapped " .. getPlayerName( thePlayer ) )
		end
	end
)
	...
	function ( thePlayer )
		if ( isElement( thePlayer ) ) and not ( isPedDead( thePlayer ) ) and ( checkPlayerACL ( client ) ) then
			...
			if ( isElementFrozen( thePlayer ) ) then
				...
				exports.VGlogging:log( "admin", client, getPlayerName( client ) .. " froze " .. getPlayerName( thePlayer ) )
				...
				exports.VGlogging:log( "admin", client, getPlayerName( client ) .. " unfroze " .. getPlayerName( thePlayer ) )
...
addEventHandler( "onServerKickPlayer", root,
	function ( thePlayer, reason )
		if ( isElement( thePlayer ) ) and ( checkPlayerACL ( client ) ) then
			if not ( isPunishAllowed( client, thePlayer ) ) then outputChatBox( "Insufficient permissions, can't punish higher staff", client, 225, 0, 0 ) return end
			exports.VGlogging:log( "admin", client, getPlayerName( client ) .. " kicked " .. getPlayerName( thePlayer ) )
	...
	function ( thePlayer, toPlayer, warpTo )
		if ( isElement( thePlayer ) ) and ( isElement( toPlayer ) ) and ( checkPlayerACL ( client ) ) then
			if ( warpTo ) then
				...
				exports.VGlogging:log( "admin", client, getPlayerName( client ) .. " warped " .. getPlayerName( thePlayer ) .. " to " .. getPlayerName( toPlayer ) )
			else
				outputChatBox( "You warped yourself to ".. getPlayerName( toPlayer ), thePlayer, 0, 225, 0 )
				exports.VGlogging:log( "admin", client, getPlayerName( client ) .. " warped to " .. getPlayerName( thePlayer ) )
			end
		...
		if ( isElement( thePlayer ) ) and ( checkPlayerACL ( client ) ) then
			if ( getPedOccupiedVehicle( thePlayer ) ) then
				if ( option == "repair" ) then
					...
					exports.VGlogging:log( "admin", client, getPlayerName( client ) .. " repaired the vehicle from " .. getPlayerName( thePlayer ) )
					...
					exports.VGlogging:log( "admin", client, getPlayerName( client ) .. " destroyed the vehicle from " .. getPlayerName( thePlayer ) )
				end
			end
		end
	end
...
addEventHandler( "onServerPlayerChangeNick", root,
	function ( thePlayer, theNick )
		if ( isElement( thePlayer ) ) and not ( getPlayerFromName( theNick ) ) and ( checkPlayerACL ( client ) ) then
			...
			exports.VGlogging:log( "admin", client, getPlayerName( client ) .. " changed the nick from " .. oldNick .. " to " .. theNick )
		else
...
addEventHandler( "onServerPlayerChangeSkin", root,
	function ( thePlayer, skinID )
		if ( isElement( thePlayer ) ) and not ( isPedDead( thePlayer ) ) and ( checkPlayerACL ( client ) ) then
			...
			exports.VGlogging:log( "admin", client, getPlayerName( client ) .. " has set the skin from " .. getPlayerName( thePlayer ) .. " to " .. skinID )
		end
	end
)
...
addEventHandler( "onServerPlayerChangeHealth", root,
	function ( thePlayer, health )
		if ( isElement( thePlayer ) ) and not ( isPedDead( thePlayer ) ) and ( checkPlayerACL ( client ) ) then
			...
			exports.VGlogging:log( "admin", client, getPlayerName( client ) .. " has set the the health from " .. getPlayerName( thePlayer ) .. " to " .. health )
		end
	end
)
...
addEventHandler( "onServerPlayerChangeArmor", root,
	function ( thePlayer, armor )
		if ( isElement( thePlayer ) ) and not ( isPedDead( thePlayer ) ) and ( checkPlayerACL ( client ) ) then
			...
			exports.VGlogging:log( "admin", client, getPlayerName( client ) .. " has set the the armor from " .. getPlayerName( thePlayer ) .. " to " .. armor )
		end
	end
)
...
addEventHandler( "onServerPlayerGiveMoney", root,
	function ( thePlayer, money )
		if ( isElement( thePlayer ) ) and ( getPlayerACLGroup ( client ) == "L6" ) then
			...
			exports.VGlogging:log( "admin", client, getPlayerName( client ) .. " has given " .. getPlayerName( thePlayer ) .. " $" .. money )
		end
	end
)
...
addEventHandler( "onServerPlayerTakeMoney", root,
	function ( thePlayer, money )
		if ( isElement( thePlayer ) ) and ( getPlayerACLGroup ( client ) == "L6" ) then
			...
			exports.VGlogging:log( "admin", client, getPlayerName( client ) .. " has taken $" .. money .. " from " .. getPlayerName( thePlayer ) )
		end
	end
)
...
addEventHandler( "onServerPlayerChangeDimension", root,
	function ( thePlayer, dimension )
		if ( isElement( thePlayer ) ) and ( checkPlayerACL ( client ) ) then
			...
			exports.VGlogging:log( "admin", client, getPlayerName( client ) .. " has set the dimension from " .. getPlayerName( thePlayer ) .. " to " .. dimension )
		end
	end
)
...
addEventHandler( "onServerPlayerChangeInterior", root,
	function ( thePlayer, interior )
		if ( isElement( thePlayer ) ) and ( checkPlayerACL ( client ) ) then
			...
			exports.VGlogging:log( "admin", client, getPlayerName( client ) .. " has set the interior from " .. getPlayerName( thePlayer ) .. " to " .. interior )
		end
	end
)
	...
	function ( thePlayer, theTime, theReason, accountOnly, IPban )
		if ( isElement( thePlayer ) ) and ( checkPlayerACL ( client ) )  then
			...
			if ( accountOnly ) then
				...
				exports.VGlogging:log( "admin", client, getPlayerName( client ) .. " has account banned " .. getPlayerName( thePlayer ) .. " for " .. exports.VGutils:secondsToTimeString ( theTime ) .. " (" .. theReason .. ")" )
				exports.VGpunish:addBan ( theReason, banTime, getPlayerAccountName( thePlayer ), false, false, getPlayerName( client ), thePlayer )
			elseif ( IPban ) then
				...
				exports.VGpunish:addBan ( theReason, banTime, getPlayerAccountName( thePlayer ), getPlayerSerial( thePlayer ), getPlayerIP( thePlayer ), getPlayerName( client ), thePlayer )
			else
				exports.VGlogging:log( "admin", client, getPlayerName( client ) .. " has banned " .. getPlayerName( thePlayer ) .. " for " .. exports.VGutils:secondsToTimeString ( theTime ) .. " (" .. theReason .. ")" )
				outputChatBox( getPlayerName( client ) .. " banned " .. getPlayerName( thePlayer ) .. " for " .. exports.VGutils:secondsToTimeString ( theTime ) .. " (" .. theReason .. ")", root, 255, 0, 0 )
				exports.VGpunish:addBan ( theReason, banTime, getPlayerAccountName( thePlayer ), getPlayerSerial( thePlayer ), false, getPlayerName( client ), thePlayer )
			end
		end
	end
)
	...
	function ( theTime, theReason, theAccount, theIP, theSerial )
		if ( checkPlayerACL ( client ) )  then
			...
			if ( isBanCreated ) then
				...
				dbExec( exports.VGlogging:getLoggingMySQLConnection(), "INSERT INTO adminlogs SET accountname=?, nickname=?, serial=?, action=?", getPlayerAccountName( client ), getPlayerName( client ), getPlayerSerial( client ), theAction2 )
	...
	function ( banID )
		if ( checkPlayerACL ( client ) ) and ( getPlayerACLGroup ( client ) == "L5" ) or ( getPlayerACLGroup ( client ) == "L6" ) then
			if ( exports.VGpunish:removeBan( tonumber( banID ) ) ) then
				exports.VGlogging:log( "admin", client, getPlayerName( client ) .. " has removed a ban with id: " .. banID )
...
function getPlayerAdminRank ( thePlayer )
	if ( isElement( thePlayer ) ) and ( adminTable[ getPlayerAccountName( thePlayer ) ] ) then
		return adminTable[ getPlayerAccountName( thePlayer ) ]
	else
		return false
	end	
end
...
function isPlayerDeveloper ( thePlayer )
	if ( isElement( thePlayer ) ) and ( getPlayerAccountName( thePlayer ) ) then
		if ( isObjectInACLGroup ( "user.".. tostring ( string.lower( getPlayerAccountName( thePlayer ) ) ), aclGetGroup ( "Developer" ) ) ) then
			return true
		else
			return false
		end
