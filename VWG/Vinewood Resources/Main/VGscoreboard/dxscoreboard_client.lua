	...
	if #scoreboardColumns ~= 0 then
				...
				for key, team in ipairs( teams ) do
					...
					for key, column in ipairs( scoreboardColumns ) do
						...
						if column.name == "name" then
							...
							for k, player in ipairs( getPlayersInTeam( team ) ) do if ( team == getTeamFromName( "Staff" ) ) and ( getElementAlpha( player ) == 0 ) then teamMemberCount = teamMemberCount -1 secretStaff = secretStaff +1 end end
							teamName = iif( teamName, tostring( teamName ), "-" )
							teamMemberCount = iif( teamMemberCount, tostring( teamMemberCount ), "0" )
							teamName = teamName .. " (" .. teamMemberCount .. ")"
							if serverInfo.allowcolorcodes then
					...
					for key, player in ipairs( players ) do
						if ( team == getTeamFromName( "Staff" ) ) and ( getElementAlpha( player ) == 0 ) then
							-- Nothinnnng
						else
		...
		if scoreboardDrawn then
			...
			if serverInfo.players and showServerInfo then
				local players = getElementsByType( "player" )
				local text = "Players: " .. tostring( (#players-secretStaff) ) .. "/" .. serverInfo.players
					...
					for key, column in ipairs( scoreboardColumns ) do
						...
						if content then
							...
							elseif type( content ) == "table" and column.name ~= "name" then
								...
								if column.name == "Flag" then
									if content == "Unknown" then
										dxDrawText( "??", topX+theX+s(1), 	y+s(1), topX+x+s(1+column.width), 	y+s(11)+dxGetFontHeight( fontscale(contentFont, scoreboardScale), contentFont ), 	tocolor( 0, 0, 0, a or 255 ), fontscale(contentFont, s(1)), contentFont, "left", "top", true, false, drawOverGUI )
										dxDrawText( "??", topX+theX, 		y, 		topX+x+s(column.width), 	y+dxGetFontHeight( fontscale(contentFont, scoreboardScale), contentFont ), 			tocolor( r or 255, g or 255, b or 255, a or 255 ), fontscale(contentFont, s(1)), contentFont, "left", "top", true, false, drawOverGUI )
									else
										dxDrawImage( topX+theX, y+s(1), 16, 11, content, 0, 0, 0, cWhite, drawOverGUI )
										dxDrawText( getElementData( element, "Country" ) or "??", topX+theX+18+s(1), 	y+s(1), topX+x+s(1+column.width), 	y+s(11)+dxGetFontHeight( fontscale(contentFont, scoreboardScale), contentFont ), 	tocolor( 0, 0, 0, a or 255 ), fontscale(contentFont, s(1)), contentFont, "left", "top", true, false, drawOverGUI )
										dxDrawText( getElementData( element, "Country" ) or "??", topX+theX+18, 		y, 		topX+x+s(column.width), 	y+dxGetFontHeight( fontscale(contentFont, scoreboardScale), contentFont ), 			tocolor( r or 255, g or 255, b or 255, a or 255 ), fontscale(contentFont, s(1)), contentFont, "left", "top", true, false, drawOverGUI )
									end
								elseif column.name == "Playtime" then
									dxDrawText( getElementData( element, "cPlaytime" ), topX+theX+s(1), 	y+s(1), topX+x+s(1+column.width), 	y+s(11)+dxGetFontHeight( fontscale(contentFont, scoreboardScale), contentFont ), 	tocolor( 0, 0, 0, a or 255 ), fontscale(contentFont, s(1)), contentFont, "left", "top", true, false, drawOverGUI )
									dxDrawText( getElementData( element, "cPlaytime" ), topX+theX, 		y, 		topX+x+s(column.width), 	y+dxGetFontHeight( fontscale(contentFont, scoreboardScale), contentFont ), 			tocolor( r or 255, g or 255, b or 255, a or 255 ), fontscale(contentFont, s(1)), contentFont, "left", "top", true, false, drawOverGUI )
								else
